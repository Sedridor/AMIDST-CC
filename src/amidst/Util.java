...
import amidst.minecraft.MinecraftClass;
...
public class Util
{
     ...
     * 
     * @param e
     *            the exception for which the stachtrace is to be shown
     */
    ...
    public static IMinecraftInterface mcInterface;
    public static IMinecraftInterface ccInterface;

    public static void setMinecraftInterfaces(IMinecraftInterface minecraftInterface, IMinecraftInterface climateControlInterface)
    {
        if (mcInterface == null)
        {
        ...
        if (ccInterface == null)
        {
            ccInterface = climateControlInterface;
        }
    }
    ...
    public static HashMap<String, String> readSrgFile()
    {
        File srgFile = new File("notch-srg.srg");
        if (!srgFile.exists())
        {
            return null;
        }

        HashMap<String, String> mcClassMap = new HashMap<String, String>();
        try
        {
            BufferedReader reader = new BufferedReader(new FileReader(srgFile));
            String line;
            ...
            try
            {
                while ((line = reader.readLine()) != null)
                {
                    ...
                    {
                        String[] var4 = line.substring(line.indexOf(":") + 1).trim().split(" ");
                        mcClassMap.put(var4[1].replace("/", "."), var4[0]);
                    }
