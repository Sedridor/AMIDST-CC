...
public class Fragment
{
    ...
    public void drawObjects(Graphics2D g, AffineTransform inMatrix)
    {
        ...
        if (!Options.instance.hideObjects.get())
        {
            for (int i = 0; i < objectsLength; i++)
            {
                if (objects[i].parentLayer.isVisible())
                {
                    ...
                    g.drawImage(objects[i].getImage(), -(objects[i].getWidth() >> 1), -(objects[i].getHeight() >> 1), objects[i].getWidth(), objects[i].getHeight(), null);
                }
            }
        }
        if (alpha != 1.0f)
