...
public class Minecraft
{
    ...
    public Minecraft(File jarFile) throws MalformedURLException
    {
        ...
        boolean[] found = new boolean[classChecks.length];
            ...
            for (int q = 0; q < classChecks.length; q++)
            {
                for (int i = 0; i < byteClasses.length; i++)
                {
                    if (!found[q])
                    {
                        ...
                        {
                            if (classChecks[q].getClass().getSimpleName().equals("CCRequire"))
                            {
                                Log.debug("Checked: " + classChecks[q].getName() + " for datas | " + classChecks[q].getClass().getSimpleName());
                            }
                            else
                            {
     ...
     * 
     * private Stack<URL> getLibraries(File path, Stack<URL> urls) { File[] files = path.listFiles(); for (int i = 0; i < files.length; i++) { if (files[i].isDirectory()) { getLibraries(files[i], urls); } else { try { Log.i("Found library: " + files[i]); urls.push(files[i].toURI().toURL()); } catch (MalformedURLException e) { e.printStackTrace(); } } } return urls; }
     */

    public void use()
    {
        File librariesJson = Options.instance.minecraftJson == null ? new File(jarFile.getPath().replace(".jar", ".json")) : new File(Options.instance.minecraftJson);
