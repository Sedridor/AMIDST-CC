...
public class MapViewer extends JComponent implements MouseListener, MouseWheelListener, KeyListener
{
    ...
    static
    {
        fragmentManager = new FragmentManager(new ImageLayer[] { new BiomeLayer(), new SlimeLayer() }, new LiveLayer[] { new GridLayer() }, new IconLayer[] { new VillageLayer(), new StrongholdLayer(), new TempleLayer(), new SpawnLayer(), new NetherFortressLayer(), playerLayer = new PlayerLayer() });
    }
    ...
    public void mouseWheelMoved(MouseWheelEvent e)
    {
        ...
        for (Widget widget : widgets)
        {
            if ((widget.isVisible()) && (mouse.x > widget.getX()) && (mouse.y > widget.getY()) && (mouse.x < widget.getX() + widget.getWidth()) && (mouse.y < widget.getY() + widget.getHeight()))
            {
    ...
    public void mouseClicked(MouseEvent e)
    {
        if (!e.isMetaDown())
        {
            ...
            MapObject object = worldMap.getObjectAt(mouse, 20.0);
